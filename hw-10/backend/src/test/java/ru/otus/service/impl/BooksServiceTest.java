...
class BooksServiceTest {
    ...
    void shouldDeleteCommentsByBookId() {
        var bookId = "1";
        var book = spy(Book.builder().id(bookId).build());
    ...
    void shouldCreateCommentSuccessfully() {
        var bookId = "1";
        var book = spy(Book.builder().id(bookId).comments(new ArrayList<>()).build());
        var comment = new Comment("new comment");
        when(bookRepository.findById(anyString())).thenReturn(Optional.of(book));

        booksServiceMongo.addComment(bookId, comment.getText());
